 ...
 * - Service Worker 런타임 캐시 규칙(app/sw.ts)과 동일한 경로 체계를 사용해야 합니다.
 */
...
export function getWasmBaseURL(): string {
  ...
  if (jsepUrl && /^https?:\/\//i.test(jsepUrl)) return stripTrailingSlash(jsepUrl);

  // 2) ORIGIN + PATHNAME
  const jsepPathname = (process.env.NEXT_PUBLIC_JSEP_PATHNAME ?? "").trim();
  const jsepOrigin = (process.env.NEXT_PUBLIC_JSEP_ORIGIN ?? "").trim() || (process.env.NEXT_PUBLIC_STORAGE_ORIGIN ?? "").trim();
