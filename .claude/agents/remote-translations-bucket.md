---
name: remote-translations
description: 원격 번역 모델 버킷 설정 전문가. 환경변수 구성, CORS 설정, 모델/ORT WASM 로딩 문제 해결 시 사용.
...
- `NEXT_PUBLIC_TRANSLATIONS_BASE` 환경변수로 원격 버킷 베이스 URL을 주입하면, 클라이언트 런타임에서 Transformers.js/onnxruntime-web이 해당 버킷에서 모든 아티팩트를 로드합니다.
- 환경변수를 비우면 기존 로컬 경로(`public/translations/**`)를 사용합니다.

## 버킷 레이아웃(예시)

- BASE: `http://localhost:5500/v0.0.1`
- 모델: `${BASE}/models/{model}/...`  (예: `.../models/Xenova/nllb-200-distilled-600M/...`)
...
```
NEXT_PUBLIC_TRANSLATIONS_BASE=http://localhost:5500/v0.0.1
```
...
- 클라이언트 환경 설정 유틸: `shared/lib/ai/env.ts`
  - `remoteBaseURL` + `preferRemote` 옵션을 해석하여 아래를 설정합니다.
    - Transformers.js: `allowLocalModels=false`, `allowRemoteModels=true`, `remoteHost`, `remotePathTemplate='/v0.0.1/models/{model}/'`
    - onnxruntime-web: `env.backends.onnx.wasm.wasmPaths='${BASE}/wasm/'`
- 호출부: `shared/lib/translator.ts`
  - `process.env.NEXT_PUBLIC_TRANSLATIONS_BASE`가 존재하면 원격 구성을 활성화합니다.
...
2) 브라우저 네트워크 패널에서 다음 요청을 확인(200 + CORS 허용):
   ...
   - `${BASE}/models/Xenova/nllb-200-distilled-600M/**` (tokenizer.json, config.json, onnx 등)
