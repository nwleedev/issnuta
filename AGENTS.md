...
- 타입스크립트 코드를 작성할 때는 any 대신 정확한 타입 또는 unknown 기반 타입 좁히기 방법을 적용하세요.
- .claude/agents 폴더 내 파일이나 CLAUDE.md 파일에 any가 있더라도 이는 타 분야에 대해서 설명하기 위한 것입니다. any를 써도 된다는 뜻이 아닙니다.

## 작업 가이드

- 인터넷 연결이 되어있지 않으면 작업을 즉시 중단해주세요.
- 요구사항이 들어오면 작업을 시작하기 전 요구사항을 단계적으로 분할해주세요.
...
- 작업을 단계적으로 분할할 때에는 Codex 또는 Claude Code가 최적의 결과를 낼 수 있는 작업 단위의 단계로 분할해주세요.
- 최적의 작업 단위를 잘 모른다면 우선 Codex 또는 Claude Code가 최적의 결과를 낼려면 어떻게 작업을 분할해야 하는지 먼저 조사해주세요.
